...
아래의 단축키 동작은 모두 나의 사용환경에 맞게 작성되어 일반적인 `vim`과 다르게
동작하는 부분이 있을 수 있다.  이는 미리 작성한 `.vimrc`를 참고하면 나의
사용환경에서 특수하게 변경된 사항을 확인할 수 있다.
...
- 커서를 `h`, `j`, `k`, `l` 키를 이용해서 움직일 수 있다. 실제로 움직임을 확인해 볼 것.
...
- `zz`를 통해 커서의 위치를 유지하면서 커서가 화면의 중앙 라인에 위치하도록 표시한다.
