...
군과 같은 방법으로 생각하여, 환에서도 준동형정리를 기술하는 것이 가능하다.
군에서는 *정규부분군에 의한 잉여군* 을 중심으로 다루었지만,
환에서는 그 대신 *아이디얼에 의한 잉여환* 에 대하여 다루고 있다고 생각하면 쉽다.

> **정리**(환의 준동형정리).
> $\phi: A\to B$를 환의 준동형이라고 하고, $\pi: A\to A/\text{Ker}(\phi)$를
> 자연스러운 준동형이라고 하자. 이 때, $\phi = \psi\circ\pi$를 만족하는
> 준동형 $\psi: A/\text{Ker}(\phi)\to B$가 유일하게 존재한다.
> 또한, $\psi$에 의하여 $G/\text{Ker}(\phi) \cong \text{Im}(\phi)$이다.

*증명*. $A$와 $B$는 환이므로, 각각 가법에 의하여 가환군을 이루고 있음을 알 수 있다.
따라서 군의 준동형정리에 의하여 가법에 대한 준동형 $\psi$가 이미 존재하므로,
이 $\psi$가 나머지 환의 준동형의 조건을 만족하고 있는지를 확인하면 된다.

$I=\text{Ker}(\phi)$라고 두자. $A/I$의 임의의 두 원소는 $x,y\in A$에 의하여
$x+I, y+I\in A/I$와 같이 나타낼 수 있다. 이 때,

$$
\psi(x+I)\psi(x+I) = \psi(\pi(x))\psi(\pi(y)) = \phi(x)\phi(y) = \phi(xy)
= \psi(\pi(xy)) = \psi(xy+I)
$$

이므로, $\psi$는 승법에 대하여 준동형이다.
$\phi$가 이미 환의 준동형이므로, $\psi(1_A+I) = \psi(\pi(1_A)) = \phi(1_A) = 1_B$
를 만족하는 것까지 확인할 수 있으므로, $\psi$는 환의 준동형이다. □

> **정리**(환에 대한 제3동형정리).
> $I$, $J$가 환 $A$의 아이디얼, $I\subset J\subsetneq A$일 때, $(A/I)/(J/I)\cong A/J$

*증명*. $\phi: A/I\ni x+I\mapsto x+J\in A/J$를 구성하여
군의 제3동형정리와 같이 적용한 것과 같이 준동형정리에 적용하면 된다. □

### 아이디얼의 대응

> **정리**(아이디얼의 대응). $I$를 환 $A$의 진아이디얼,
> $\pi: A\to A/I$를 자연스러운 준동형이라고 하자.
> $A/I$의 아이디얼 전체의 집합을 $X$, $I$를 포함하는 $A$의 아이디얼 전체의 집합을 $Y$로
> 두면, 다음 두 사상 $\phi$와 $\psi$가 존재하며,
>
> - $\phi: X\ni \bar{J}\mapsto \pi^{-1}(\bar{J})\in Y$
> - $\psi: Y\ni J\mapsto \pi(J)\in X$
> 
> $\phi$와 $\psi$는 서로의 역사상이다. 따라서 $X$와 $Y$간에는 일대일 대응이 존재한다.

*증명*. 군의 준동형정리에 의하여 $\bar J\in X$라면, $\pi^{-1}(J)$가
$A$의 가법에 대한 부분군, $J\in Y$라면 $\pi(J)$가 $A/I$의 가법에 대한 부분군이며,
두 사상이 서로의 역사상을 이루고 있음을 알 수 있었다.
따라서 이 정리를 보이기 위해서는 추가적으로 $\bar J\in X$라면 $\pi^{-1}(J)$가
$A$의 아이디얼, $J\in Y$라면 $\pi(J)$가 $A/I$의 아이디얼이라는 것을 보이면 된다.

$\bar J\in X$에 대하여 $J=\pi^{-1}(\bar J)$로 두자.
$a\in A$, $x\in J$일 때, $\pi(ax) = \pi(a)\pi(x)$이고, $\pi(x)\in \bar J$,
$\bar J$는 아이디얼이므로, $\pi(ax)\in \bar J$. 따라서 $ax\in J$이다.
동시에 $J$는 이미 가법에 대하여 부분군을 이루고 있으므로,
$J$는 아이디얼이다.

반대로 $J\in Y$에 대하여 $\bar J = \pi(J)$로 두자.
그렇다면 $x\in J$일 때, $\bar x = x + I \in \pi(J) = \bar J$이다.
마찬가지로 $a\in A$일 때, $\bar a = a + I\in A/I$이라면,
$J$가 아이디얼이므로 $ax\in J$,
$\bar a\bar x = \overline{ax} = ax + I \in \pi(J) = \bar J$.
동시에 $\bar J$는 이미 가법에 대하여 부분군을 이루고 있으므로,
$\bar J$는 아이디얼이다. □

### 준동형의 분해

> **정리**. $\phi: A\to B$를 환의 준동형, $I$가 $A$의 아이디얼,
> $\pi: A\to A/I$을 자연스러운 준동형이라고 할 때, 다음 두 조건은 동치이다.
>
> 1. $I\subset \text{Ker}(\phi)$
> 1. $\phi = \psi\circ\pi$를 만족하는 준동형 $\psi: A/I\to B$가 존재한다.
